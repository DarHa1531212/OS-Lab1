...
int CalculerChunk(int nbrThreads, int noChunk, vector<vector<vector<unsigned long int>>>vecteurProduitMatrice) {
    ...
    unsigned long int tailleChunk = vecteurTemporaire.size() / (nbrThreads * 2);
    Chunk chunk;
    ...
    for (int i = 0; i < tailleChunk; i++)
    {
        ...
        chunk.chunkDeMatrices.at(i) = CalculerProduitScalaire(vecteurProduitMatrice.at(tailleChunk * noChunk), vecteurProduitMatrice.at(vecteurTemporaire.size() - (tailleChunk * noChunk) - i - 1));
    ...
    for (int i = 0; i < tailleChunk; i++)
    {
        position = tailleChunk*noChunk + i;
        vecteurTemporaire.at(position) = chunk.chunkDeMatrices.at(i);
    }
...
int AffecterMatricesPourProduitMatriciel(int nbrDeThreads ,vector<vector<vector<unsigned long int>>>vecteurProduitMatrice)
{
    ...
    vector<thread> vecteurThreads;

    for(int i = 0; i > nbrDeThreads; i++) {

        vecteurThreads.emplace_back([&](){CalculerChunk(nbrDeThreads, i, vecteurProduitMatrice);});
...
int main() {
	...
	{
		vecteurProduitMatriciel[d]=CalculerProduitScalaire(vecteurDeMatricesOriginal[d], vecteurDeMatricesTransposees[d]);
